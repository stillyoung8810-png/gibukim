...
export type RewardAdLoadResult = { readonly type: 'loaded' } | { readonly type: 'failed' };
...
export function createTossRewardAdGateway(
  ...
  return {
    ...
    preloadNext(): Promise<RewardAdLoadResult> {
      ...
      const adGroupId = getRewardAdGroupId(normalizedAdGroupIds, nextAdGroupIndex);
      ...
      const settle = (result: RewardAdLoadResult) => {
        ...
        isSettled = true;
        cleanup();
        state = result.type === 'loaded' ? { type: 'loaded', adGroupId } : { type: 'idle' };
    ...
    showPreloaded(): Promise<RewardedAdResult> {
      ...
      const settle = (result: RewardedAdResult) => {
        ...
        if (result.type === 'earnedReward') {
          nextAdGroupIndex = (nextAdGroupIndex + 1) % normalizedAdGroupIds.length;
        }
        state = { type: 'idle' };
        deferred.resolve(result);
      };
...
function getRewardAdGroupId(adGroupIds: NonEmptyRewardAdGroupIds, index: number): string {
